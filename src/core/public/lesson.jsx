...
export default function Lesson() {
  ...
  useEffect(() => {
    const fetchQuizzes = async () => {
      try {
        const response = await fetch(
          `https://localhost:3000/api/quiz/getquiz?instrument=${encodeURIComponent(
    ...
    const fetchCompletedLessons = async () => {
      try {
        const response = await fetch(
          "https://localhost:3000/api/completed/getcompleted",
          {
            method: "GET",
            headers: {
  ...
  const filteredQuizzes = quizzes.filter(
    (quiz) => quiz.instrument.toLowerCase() === selectedCategory.toLowerCase()
  );
  const uniqueDays = [...new Set(filteredQuizzes.map((quiz) => quiz.day))].sort(
    (a, b) => {
  ...
  const completedDaysCount = uniqueDays.filter((day) =>
    completedLessons.some(
      (lesson) =>
        lesson.day === day &&
        lesson.instrument.toLowerCase() === selectedCategory.toLowerCase()
    )
  ).length;
  ...
  const isDayWrong = (day) => {
    return incorrectAnswers.some(
      (answer) =>
        answer.day === day &&
        answer.instrument.toLowerCase() === selectedCategory.toLowerCase()
    );
  };
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-gray-900 via-indigo-900 to-purple-900">
      ...
      {/* Enhanced Profile Picture - Moved to top right of main content */}
