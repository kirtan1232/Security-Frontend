...
export default function Profile() {
  ...
  const handlePasswordChange = async () => {
    ...
    if (passwordScore < 2) {
      setError(
        "Password is too weak! Try a mix of letters, numbers, symbols, and make it longer."
      );
      return;
    }
    ...
    if (
      safeOldPassword &&
      safeNewPassword &&
      safeOldPassword === safeNewPassword
    ) {
      toast.error("You can't put the same password as before.", {
        position: "top-right",
        autoClose: 3000,
      });
      return;
    ...
    let freshCsrfToken = "";
    try {
      const csrfRes = await fetch("https://localhost:3000/api/csrf-token", {
        credentials: "include",
      });
    ...
    try {
      const response = await fetch(
        "https://localhost:3000/api/auth/update-profile",
        {
          method: "PUT",
          headers: {
      ...
      if (!response.ok) {
        ...
        if (
          ...
          toast.error("You can't use the same password as your old one.", {
            position: "top-right",
            autoClose: 3000,
          });
          return;
    ...
    try {
      ...
      setImagePreview(
        data.user.profilePicture
          ? `https://localhost:3000/${data.user.profilePicture}`
          : null
      );
    } catch (err) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-gray-900 via-indigo-900 to-purple-900">
      ...
      {/* Main Content */}
      <main className="flex-1 p-6 flex justify-center items-start mt-6 relative z-10">
